...
class MemoryTrustLevel(Enum):
    ...
    VERIFIED = "verified"  # Verified by system or admin
    USER_INPUT = "user_input"  # Direct user input
    AGENT_GENERATED = "agent_generated"  # Generated by a trusted agent
    EXTERNAL = "external"  # From external documents/URLs
    UNTRUSTED = "untrusted"  # Unknown or suspicious source


# Score multipliers for trust-weighted retrieval
TRUST_WEIGHT = {
    MemoryTrustLevel.VERIFIED: 1.0,
    MemoryTrustLevel.USER_INPUT: 1.0,
    MemoryTrustLevel.AGENT_GENERATED: 0.9,
    MemoryTrustLevel.EXTERNAL: 0.6,
    MemoryTrustLevel.UNTRUSTED: 0.3,
}
...
class MemoryEntry:
    ...
    trust_level: MemoryTrustLevel = MemoryTrustLevel.AGENT_GENERATED
...
class RetrievalPipeline:
    ...
    def retrieve(
        ...
        for result in results:
            ...
            # Skip quarantined memories unless explicitly included
            if memory.quarantined:
                filtered_count += 1
                continue

            # Apply trust-weighted scoring
            trust_weight = TRUST_WEIGHT.get(memory.trust_level, 0.5)
            weighted_score = result.score * trust_weight
